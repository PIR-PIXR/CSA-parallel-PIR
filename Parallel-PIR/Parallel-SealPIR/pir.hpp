...
/*void get_pir_params(uint64_t ele_num, uint64_t ele_size, uint32_t d,
                    const seal::EncryptionParameters &enc_params, bool enable_symmetric = false,
                    bool enable_batching = true, bool enable_mswitching = true);*/
...
seal::GaloisKeys *
deserialize_galoiskeys(std::string s,std::shared_ptr<seal::SEALContext> context);
