...
///home/dowload/valgrind-3.21.0/coregrind/valgrind -s --tool=memcheck --leak-check=full --show-leak-kinds=all /mnt/e/Git/Projects/PIR-PIXR/Ethereum-Project/pirmessage_client ls -l
...
std::vector<uint8_t> get_seal_params(string jsonFile,uint64_t &number_of_items,uint64_t &size_per_item,Document &doc) {    
    ...
    // Read the entire file into a string
    cout << "Loading the database (this may take some time) ..." << endl;
...
PIRClient create_pir_params(string encryptionFile,PirParams &pir_params, uint64_t number_of_items,uint64_t size_per_item){
  ...
  //encryptionFile = "encryption_parameters.bin";
  uint32_t logt = 20;
  uint32_t d = 2;
